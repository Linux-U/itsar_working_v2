...
#Read prot.table file

prot.table <- read.table(file = "prot.table.txt",
...
prot.table <- filter(prot.table, Reverse != "+", Potential.contaminant != "+", Only.identified.by.site != "+")



#Filter for protines with 2 or more unique peptides

prot.table <- filter(prot.table, Unique.peptides > 1)



#Filter for protines with 2 or more unique peptides

prot.table <- filter(prot.table, Reporter.intensity.count.1 > 11)
...
rownames(prot.table) <- prot.table[,"name"]



#Build an ExpressionSet from prot.table data.frame
...
assaydata <- data.matrix(prot.table[,c(32:40,42)]) #channel 10 removed, outlier in PCA
...
featuretable <- prot.table[c(1:31,43:68)]
...
qstats_obj <- p.adjust(ebayes_limma$p.value[,"TreatmentD"],
                       method = "BH", length(ebayes_limma$p.value[,"TreatmentD"]))
sum(qstats_obj < 0.05)
hist(qstats_obj,col=2)


#Make a prot.table object with limma p values, q values, and expression diffrences
...
prot.table[,c(32:40,42)] <- norm_edata[,1:10]
prot.table_limma <- merge(prot.table, stats_table, by=0)

write.csv(prot.table_limma, file ="prot.table_limma_vehicle_SVA.csv") #reconstruct prot.table file with limma/SVA results

writeLines(capture.output(sessionInfo()), "sessionInfo.txt") #session info
